...
contract GroveFarm_UsdsGroveFarmingDeployScript is Script {
    ChainlogLike internal constant chainlog = ChainlogLike(0xdA0Ab1e0017DEbCd72Be8599041a2aa3bA7e740F);

    string internal constant NAME = "grove-farm/usds-grove-farming-deploy";
    ...
    function run() external {
        ...
        address grove = reader.envOrReadAddress("FOUNDRY_GROVE", ".grove");
        ...
        if (rewards == address(0)) {
            rewards = StakingRewardsDeploy.deploy(
                StakingRewardsDeployParams({owner: admin, stakingToken: usds, rewardsToken: grove})
            );
        }
        ...
        if (dist == address(0)) {
            dist = VestedRewardsDistributionDeploy.deploy(
                VestedRewardsDistributionDeployParams({
                    deployer: msg.sender, owner: admin, vest: vest, rewards: rewards
                })
            );
        }
        ...
        ScriptTools.exportContract(NAME, "rewardsToken", grove);
