...
contract GroveFarm_GroveFarmingCheckDeploymentScript is Script {
    function run() external returns (bool) {
        Reader deps = new Reader(ScriptTools.loadDependencies());

        address admin = deps.readAddress(".admin");
        address grove = deps.readAddress(".rewardsToken");
        ...
        require(VestedRewardsDistributionLike(dist).gem() == grove, "VestedRewardsDistribution/invalid-gem");
        require(
            VestedRewardsDistributionLike(dist).stakingRewards() == rewards,
            "VestedRewardsDistribution/invalid-staking-rewards"
        );

        require(StakingRewardsLike(rewards).rewardsToken() == grove, "StakingRewards/invalid-rewards-token");
        ...
        require(DssVestWithGemLike(vest).gem() == grove, "DssVest/invalid-gem");
